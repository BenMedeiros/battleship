...
import {GamePhase} from "../server/statuses.js";
...
export class AiBasic {
  ...
  doPlannedMove() {
    console.log(this);
    if (this.gameProxy.gameState.phase === GamePhase.place_ships) {
      const plannedMove = planPlaceShips(this.gameProxy.getGameConfig(), this.gameProxy.getPlayer());
      if (plannedMove === null) throw new Error('All ships placed, no planning');
...
function planPlaceShips(gameConfig, player) {
